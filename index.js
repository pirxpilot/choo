...
export default function choo(opts = {}) {
  ...
  function start() {
    assert(
      ...
      'choo.start: window was not found. .start() must be called in a browser, use .toString() if running in Node'
    );
    ...
    emitter.prependListener(
      ...
      nanoraf(() => {
        ...
        assert(
          tree.nodeName === newTree.nodeName,
          `choo.render: The target node <${tree.nodeName.toLowerCase()}> is not the same type as the new node <${newTree.nodeName.toLowerCase()}>.`
        );
  ...
  function mount(selector) {
    ...
    assert(
      ...
      'choo.mount: selector should be type String or HTMLElement'
    );
    ...
    documentReady(() => {
      ...
      assert(
        tree.nodeName === newTree.nodeName,
        `choo.mount: The target node <${tree.nodeName.toLowerCase()}> is not the same type as the new node <${newTree.nodeName.toLowerCase()}>.`
      );
  ...
  function toString(location, state = {}) {
    ...
    assert(
      typeof window !== 'object',
      'choo.mount: window was found. .toString() must be called in Node, use .start() or .mount() if running in the browser'
    );
